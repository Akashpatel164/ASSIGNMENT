...
create database people_address;
use people_address;
CREATE TABLE PEOPLE (id INTEGER  , name CHAR(1));
...
-- 10.1 Join table PEOPLE and ADDRESS, but keep only one address information for each person (we don't mind which record we take for each person). 
    ...
    SELECT
PEOPLE.id, PEOPLE.name, TEMP.address FROM PEOPLE LEFT JOIN
(SELECT id, MAX(address) as address FROM ADDRESS GROUP BY id)AS TEMP ON PEOPLE.id = TEMP.id; 
