...
-- 8.3 Obtain the names of all physicians that have performed a medical procedure that they are certified to perform, but
  ---   such that the procedure was done at a date (Undergoes.Date) after the physician's certification expired (Trained_In.CertificationExpires);
  
SELECT P.NAME 
FROM
	Physician AS P,Trained_In T,Undergoes AS U 
WHERE
...
-- 8.5 Obtain the information for appointments where a patient met with a physician other
 --- than his/her primary care physician. Show the following information: Patient 
...
SELECT N.NAME FROM	Nurse N WHERE EmployeeID IN (SELECT OC.Nurse FROM On_Call OC, Room R 
	WHERE OC.BlockFloor = R.BlockFloor AND OC.BlockCode = R.BlockCode AND R.RoomNumber = 123 );
...
SELECT Pt.NAME,PhPCP.NAME FROM Patient Pt,Physician PhPCP 
    WHERE Pt.PCP = PhPCP.EmployeeID AND EXISTS ( SELECT * FROM Prescribes Pr WHERE Pr.Patient = Pt.SSN AND Pr.Physician = Pt.PCP ) 
	     AND EXISTS (SELECT * FROM Undergoes U, Procedures Pr WHERE U.Procedures = Pr.CODE AND U.Patient = Pt.SSN AND Pr.Cost > 5000 ) 
	       AND 2 <= (SELECT COUNT( A.AppointmentID ) FROM Appointment A, Nurse N WHERE A.PrepNurse = N.EmployeeID AND N.Registered = 1 ) 
